...
define(function(require) {
	...
	return function() {
		...
		function triggerTouch(touches, element, type) {
			var touchlist = [],
				event = new PointerEvent('pointer' + type);

			event.initEvent('pointer' + type, true, true);
		...
		function triggerGesture(element, startPos, startTouches, gesture, done) {
			...
			function gestureLoop() {
				...
				if (isFirst) {
					triggerTouch(touches, element, 'down');
				} else if (isLast) {
					triggerTouch(touches, element, 'up');
					return done(touches);
				} else {
		...
		function toucherGremlin(done) {
			...
			function logGremlin(touches, details) {
				...
				if (config.logger && typeof config.logger.log == 'function') {
					config.logger.log('gremlin', 'pointerEvent   ', touchType, 'at', posX, posY, details);
				}
				done();
			}
		}
